...
int main () {
	...
	*/
	...
	size_t test = tools::count(vector.begin(), vector.end());
	if (test==vector.size()){cout<<"test de la fonction count concluant"<<endl;}
	else {cout<<"test de la fonction count :"<<endl<<"	elle a compté "<< test <<" mots différents alors qu'il aurais du en voir"<< vector.size() <<endl;}
